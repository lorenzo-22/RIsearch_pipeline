...
uv sync
```
Requires Python ≥3.14 and ViennaRNA 2.7.2 (installed via `uv sync`). The Rust `RIsearch` binary (separate from this Python pipeline) is expected at `~/.cargo/bin/RIsearch`.
...
```
The `gw` entrypoint is an alias for `risearch-pipeline`.
...
services/                       ← stateless processing units
  risearch_service.py           ← wraps Rust RIsearch binary (index + search)
  risearch_parser.py            ← parses RIsearch TSV output into Polars DataFrames

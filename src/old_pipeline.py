    ...
    for temp_file in TEMP_FILES:
        try:
            ...
            sys.stderr.write(
                "#WARNING: "
                + temp_file
                + " could not be removed. It might not exist.\n"
            )
    if failed:
...
def get_parser():
    ...
    parser.add_argument(
        "--column",
        help="Column to use in transcriptome input (default: 2)",
        type=int,
        default=2,
    )
    ...
    group_pos = parser.add_argument_group(
        "# on-target Option Group 1 #\nDetect the on-target with given transcript ID or genomic location"
    )
    group_pos.add_argument(
        "-oi", metavar="<str>", help="on-target transcript ID", type=str, default="None"
    )  # ,required=True)
    group_pos.add_argument(
        "-on",
    ...
    group_on_pre = parser.add_argument_group(
        "# on-target Option Group 2 #\n# Predict on-targets and compute accessibility on given fasta file"
    )
    group_on_pre.add_argument(
        "-of",
        metavar="<file>",
    ...
    group_on = parser.add_argument_group(
        "# on-target Option Group 3 #\nUse precomputed ontarget predictions and accessibilities"
    )
    group_on.add_argument(
        "-op",
        metavar="<file>",
...
def split_prediction_line(given_line):
    ...
    if len(given_cols) == 9 or len(given_cols) == 8:  # RIsearch output file
        ...
        chromosome, spos, epos, tid, energy, strand, expression = given_cols
        return (
            chromosome,
            int(spos),
            int(epos),
...
def get_transcriptome_intersect(
    ris_file,
    trans_file,
    feature,
    score_type,
    ...
    intersection_file = "%s_intersected_%s.targets" % (ris, trans)
    intersection_column_file = "%s_intersected_%s.%d.targets" % (ris, trans, column)
    # For gw mode, use full intersection file; for tw mode, use column subset
    if Rtype == "gw":
        if os.path.exists(intersection_file):
            sys.stderr.write(
                "#INFO: %s already there, not recreating\n" % intersection_file
            )
        ...
        if os.path.exists(intersection_file):
            ...
            with open(intersection_column_file, "wb") as icf:
                p1 = Popen(
                    ["cut", "-f1-6,%d" % (column + 5), intersection_file], stdout=icf
                )
                p1.communicate()
            return intersection_column_file
    ...
    with open(ris_bed, "wb") as ris_out_f:
            ...
            ris_out_f.write(
                "\t".join(
                    [
                        ...
                        ris_cols[3]
                        + "_"
                        + str(int(ris_cols[2]) - int(ris_cols[1]) + 1),
                        "1",
                        ris_cols[6],
                        ris_cols[7] + "\n",
                    ]
    ...
    if (
        ".gtf" in trans_file
        or ".gff" in trans_file
        or ".GTF" in trans_file
        or ".GFF" in trans_file
    ):
        in_f = fn_open(trans_file)
        with open(trans_bed, "wb") as out_f:
            for line in in_f:
                ...
                if "feature" in t_dic.keys() and t_dic["feature"] == feature:
                    if (
                        "seqname" in t_dic.keys()
                        and "start" in t_dic.keys()
                        and "end" in t_dic.keys()
                        and "strand" in t_dic.keys()
                        and score_type in t_dic.keys()
                    ):
                        g_id = (
                            t_dic["gene_id"]
                            if "gene_id" in t_dic.keys()
                            else t_dic["seqname"]
                        )
                        t_id = (
                            t_dic["transcript_id"]
                            if "transcript_id" in t_dic.keys()
                            else g_id
                        )
                        ...
                        sys.stderr.write(
                            "WARNING: Skipping line in .gtf/.gff transcriptome file, one or more fields missing (required fields: seqname, start, end, strand, "
                            + score_type
                            + ")\n"
                        )
        ...
        trans_file.endswith(".bed")
        or trans_file.endswith(".bed.gz")
        or trans_file.endswith(".BED")
        or trans_file.endswith(".BED.gz")
    ):
        in_f = fn_open(trans_file)
        ...
        if len(t_cols) < 6:
            sys.stderr.write(
                "#ERROR: Transcriptome .bed file has less than 6 columns\n#TERMINATING\n"
            )
            pipeline_exit(failed=True)
        in_f.close()
        ...
        sys.stderr.write(
            "ERROR: Unknown type for Transcriptome file (only .gtf,.gff or .bed)\n#TERMINATING\n"
        )
        pipeline_exit(failed=True)
    ...
    if Rtype == "gw":
        ...
        with open(intersection_file + ".temp", "wb") as output:
            subprocess.call(
                "bedtools intersect -wo -a %s -b %s" % (trans_bed, ris_bed),
                stdout=output,
                shell=True,
            )
        ...
        with open(intersection_file + ".temp") as int_in_f:
            with open(intersection_file, "wb") as int_out_f:
                ...
                for line in int_in_f:
                    ...
                    if not (
                        prev == cur
                    ):  # or (prev[0] == cur[0]  and 'hap' in prev[1] and 'hap' in cur[1])):
                        int_out_f.write(line)
                    prev = cur

        if sort:
            with open(intersection_file + ".sorted", "wb") as output:
                subprocess.call(
                    "sort -k9,9 -k14,14 -k10n %s" % (intersection_file),
                    stdout=output,
                    shell=True,
                )
        ...
        with open(intersection_file, "wb") as out_f:
        ...
        if sort:
            with open(intersection_file + ".sorted", "wb") as output:
                subprocess.call(
                    "sort -k1,1 -k6,6 -k2n %s" % (intersection_file),
                    stdout=output,
                    shell=True,
                )
            intersection_file = intersection_file + ".sorted"
        ...
        sys.stderr.write(
            "#ERROR: Wrong type selection for RIsearch2 results. Select gw or tw\n#TERMINATING\n"
        )
        pipeline_exit(failed=True)

    # For gw mode, return the full intersection file (>12 columns) which has energy in cols[-2]
    # For tw mode, create a 7-column subset
    if Rtype == "gw":
...
def create_acc_bin(seq_file, seq_name, outfile, w=80, l=40):
    ...
    try:
        ...
        sub_output = subprocess.Popen(
            acc_cmd, shell=True, stdout=subprocess.PIPE
        ).communicate()
    except:
        sys.stderr.write(
            "#ERROR: RNAplfold could not be executed for on-target.\n#TERMINATING\n"
        )
        sys.stderr.write("Unexpected error: " + str(sys.exc_info()) + "\n")
        pipeline_exit(failed=True)

    with open(outfile, "wb+") as out_f:
        with open(score_file) as in_f:
            for sc_line in in_f:
...
def get_opening_energy(
    tid, spos, epos, strand, given_bin="", bin_folder="", acc_f=None
):
    ...
    try:
        ...
        sys.stderr.write(
            "#WARNING: problem reading %s, seek failed, %d, %d, %d, %d\n"
    ...
    try:
        ...
        sys.stderr.write(
            "#WARNING: problem reading %s, read failed, %d, %d, %d, %d\n"
            % (filepath, spos, epos, offset, i_val_y)
        )
        vx = "%c" % 0
        pass
    try:
        value = float(ord(vx)) / 10
    except:
        sys.stderr.write(
            "#WARNING: problem reading %s, float failed (%s), %d, %d, %d, %d\n"
...
def get_predictions(
    ...
    if qid != "" and ris2 != "" and seq_file != "":
        ...
        try:
            ...
            sys.stderr.write(
                "#ERROR: file is not found " + seq_file + "\n#TERMINATING\n"
            )

        if seq == "":
            sys.stderr.write(
                "#ERROR: siRNA fasta file do not contain sequence with ID: "
                + args.q
                + "\n#TERMINATING\n"
            )
            pipeline_exit(failed=True)
        ...
        try:
            ...
            out_ris2_suf = subprocess.Popen(
                ris_cmd, shell=True, stdout=subprocess.PIPE
            ).communicate()[0]
            ...
            out_ris2_pre = subprocess.Popen(
                ris_cmd, shell=True, stdout=subprocess.PIPE
            ).communicate()[0]

            f = gzip.open("risearch_" + temp + ".out.gz")
            minimum_noaccs = float(f.readline().rstrip().split("\t")[-1])
            for line in f:
                temp_eng = float(line.rstrip().split("\t")[-1])
                minimum_noaccs = (
                    temp_eng if temp_eng < minimum_noaccs else minimum_noaccs
                )
            f.close()
            sys.stderr.write(
                "#GETPREDICTIONS: siRNA sequence:"
                + seq
                + " minimum:"
                + str(minimum_noaccs)
                + "\n"
            ...
            sys.stderr.write(
                "#ERROR: Could not run RIsearch2 to obtain E_min. Link to RIsearch2 executable might be broken.\n"
            )
            sys.stderr.write("Unexpected error: " + str(sys.exc_info()) + "\n")
            pipeline_exit(failed=True)
    ...
    p_f = None
    try:
        p_f = gzip.open(filepath) if filepath[-3:] == ".gz" else open(filepath)
    except:
        sys.stderr.write(
    ...
    for p_line in p_f:
        ...
        if split_prediction_line(p_line) == None:
            continue
        chromosome, spos, epos, strand, energy, expression, gid, tid = (
            split_prediction_line(p_line)
        )
        if (threshold is None) or (energy < threshold):
            if energy < minimum_noaccs:
                minimum_noaccs = energy
            if include_opening_energies:
                if ((less is False) or (strand == "+")) and (
                    (chromosome_choice == "") or (chromosome == chromosome_choice)
                ):
                    open_eng, temp_acc_f = get_opening_energy(
                        chromosome,
                        spos,
                        epos,
                        strand,
                    ...
                    if open_eng >= 0.0:
                        if (
                            (thr_after_acc is False)
                            or (threshold is None)
                            or ((energy + open_eng) < threshold)
                        ):
                            predictions.append(
                                (
                                    chromosome,
                        ...
                        exc_open += 1
                        sys.stderr.write(
                            "Can not get opening energy for "
                            + " ".join(
                                [
                                    chromosome,
                                    str(spos),
    ...
    sys.stderr.write(
        "#GETPREDICTIONS: "
        + str(exc_less)
        + " predictions excluded (sense/antisense strand/chromosome choice); "
        + str(exc_open)
        + " excluded (opening energy); "
        + str(exc_thr)
        + " excluded (energy threshold)\n"
    )
    return predictions, on_target_predictions, minimum, minimum_noaccs


# Update the predictions list for on-targets
def add_update_on_targets(
    on_target_pred_file,
    on_target_acc_bin,
    predictions,
    on_target_predictions,
    ...
    p_f = gzip.open(on_target_pred_file)
    for p_line in p_f:
        if split_prediction_line(p_line) == None:
            continue
        chromosome, spos, epos, strand, energy, expression, gid, tid = (
        ...
        if (strand == "+") and ((threshold is None) or (energy < threshold)):
            if include_opening_energies:
                ...
                predictions.append(
                    (chromosome, spos, epos, strand, energy, 0.0, oexp, gid, tid)
                )
    p_f.close()
...
def apply_tp_threshold(
    predictions,
    on_target_predictions,
    threshold_per,
    min,
    min_na,
    thr_after_openen=False,
):
    new_predictions = []
    new_on_targets = []
...
def get_z(predictions, minimum, minimum_noaccs, parameter_list):
    ...
    for (
        ...
        for par in parameter_list:
            ...
            value_z_noaccs[par] += expression * exp(
                -1.0 * par_beta * assigned_energy_noaccs
            )
...
def get_probabilities(
    predictions, on_targets, minimum, minimum_noaccs, parameter_list, off_ps=False
):
    prob_scores = {}
    prob_scores_noaccs = {}
    kb = 0.001987
    ...
    value_z, value_z_noaccs = get_z(
        predictions, minimum, minimum_noaccs, parameter_list
    )
        ...
        for (
            ...
            for par in parameter_list:
                if type(par) is float:
                    ...
                    continue
                prob_scores[par].append(
                    (expression * exp(-1.0 * par_beta * assigned_energy)) / value_z[par]
                )
                prob_scores_noaccs[par].append(
                    (expression * exp(-1.0 * par_beta * assigned_energy_noaccs))
                    / value_z_noaccs[par]
                )
    ...
    for par in parameter_list:
        ...
        if type(par) is float:
            for i in on_targets:
                ...
                assigned_energy_noaccs = (par * (energy + 10)) - 10
                on_target_prob += (
                    expression * exp(-1.0 * par_beta * assigned_energy)
                ) / value_z[par]
                on_target_prob_noaccs += (
                    expression * exp(-1.0 * par_beta * assigned_energy_noaccs)
                ) / value_z_noaccs[par]
        elif type(par) is tuple and len(par) == 2:
            (alpha, gamma) = par
...
def get_off_target_dic(off_target_bed_path):
    off_dic = {}
    with open(off_target_bed_path) as off_f:
        for off_line in off_f:
            off_cols = off_line.rstrip().split()
            if off_cols[3] in off_dic.keys():
                off_dic[off_cols[3]].append(
                    off_cols[0], off_cols[1], off_cols[2], off_cols[5]
                )
            else:
                off_dic[off_cols[3]] = [
                    (off_cols[0], off_cols[1], off_cols[2], off_cols[5])
...
def report(
    ...
    for par in parameter_list:
        ...
        out.write(
            ...
            + str(on_target_prob)
            + "; Pon_noacc: "
            + str(on_target_prob_noaccs)
            + "; Poff: "
            + str(1.0 - on_target_prob)
            + "; Poff_noacc: "
            + str(1.0 - on_target_prob_noaccs)
            + "; Z: "
            + str(z_val[par])
            + "; Z_noacc: "
            + str(z_val_na[par])
            + "; Zoff: "
            + str(z_val[par] * (1.0 - on_target_prob))
            + "; Zoff_noacc: "
            + str(z_val_na[par] * (1.0 - on_target_prob_noaccs))
            + "\n"
        )
    ...
    if verbose:
        ...
        for i in range(len(predictions)):
            chrom, spos, epos, strand, energy, open_eng, expression, gid, tid = (
                predictions[i]
            )
            id_key = "\t".join([gid, tid])
        ...
        for id_key in off_probs.keys():
            ...
            for par in parameter_list:
                if type(par) is float or (type(par) is tuple and len(par) == 2):
                    out.write(
                        "\t"
                        + str(off_probs[id_key][par])
                        + "\t"
                        + str(off_probs_noaccs[id_key][par])
                    )
            out.write("\n")
...
def run_pipeline(args):
    ...
    if args.op == "":
        ...
        if args.of == "":
            sys.stderr.write(
                "#CHECKPOINT: On-target fasta file is not given."
                + " On-target info will be recognized with given Transcript ID or genomic location\n"
            )
        elif args.os == "":
            sys.stderr.write(
                "#CHECKPOINT: siRNA fasta file is not given."
                + " On-target info will be recognized with given Transcript ID or genomic location\n"
            )
        elif args.rx == "":
            sys.stderr.write(
                "#ERROR: RIsearch2 executable command is missing\n#TERMINATING\n"
            )
            pipeline_exit(failed=True)
        else:
            sys.stderr.write(
                "#CHECKPOINT: On target prediction-file is computed with given sequence files\n"
            )
            on_target_name = ""
            with open(args.of) as of_f:
                for of_line in of_f:
                    if of_line[0] == ">":
            ...
            if on_target_name == "":
                sys.stderr.write(
                    "#ERROR: wrong on-target sequence file (no sequence name)\n#TERMINATING\n"
                )
                pipeline_exit(failed=True)
            ...
            try:
                ...
                if args.rp != "":
                    ...
                    for param in args.rp.split(";"):
                        ...
                        if len(par_cols) > 2 or len(par_cols) == 0:
                            sys.stderr.write(
                                "#ERROR: wrong RIsearch2 parameters\n#TERMINATING\n"
                            )
                ...
                sys.stderr.write(
                    "#ERROR: failed running RIsearch2 on on-target\n#TERMINATING\n"
                )
                sys.stderr.write("Unexpected error: " + str(sys.exc_info()) + "\n")
                pipeline_exit(failed=True)
    ...
    if args.oa == "":
        if on_predictions_file == "" or args.p == "":
            sys.stderr.write(
                "#CHECKPOINT: On-target opening energies are not relevant/needed\n"
            )
        elif args.of == "":
            sys.stderr.write(
                "#ERROR: On-target fasta file is not given.\n#TERMINATING\n"
            )
            pipeline_exit(failed=True)
        else:
            acc_w = 80
            ...
            if args.ap != "":
                for inf in args.ap.split(";"):
                    if inf[0] == "W":
                        ...
                        acc_l = int(inf[2:])
                sys.stderr.write(
                    "#CHECKPOINT: RNAplfold parameters W="
                    + acc_w
                    + " L="
                    + acc_l
                    + "\n"
            ...
            try:
                create_acc_bin(
                    args.of, on_target_name, on_accessibility_file, acc_w, acc_l
                )
            except:
                sys.stderr.write(
                    "#ERROR: failed while computing accessibility on on-target\n#TERMINATING\n"
                )
                sys.stderr.write("Unexpected error: " + str(sys.exc_info()) + "\n")
                pipeline_exit(failed=True)
    ...
    if args.t != "":
        try:
            off_predictions_file = get_transcriptome_intersect(
                off_predictions_file,
                args.t,
                args.feature,
                args.expmetric,
                args.type,
                column=args.column,
                sort=args.sort,
                keep=args.intersection,
                less=args.less,
            )
        except:
            sys.stderr.write(
                "#ERROR: failed while transcriptome intersection\n#TERMINATING\n"
            )
            sys.stderr.write("Unexpected error: " + str(sys.exc_info()) + "\n")
            pipeline_exit(failed=True)
        sys.stderr.write(
            "#CHECKPOINT: Predictions and Transcriptome intersection is done and saved to "
            + off_predictions_file
            + " "
            + on_predictions_file
            + "\n"
    ...
    try:
        pres, on_targets, min_p, min_na = get_predictions(
            off_predictions_file,
            on_id=ont_id,
            on_chrom=ont_c,
            on_spos=ont_s,
            ...
            on_strand=ont_str,
            threshold=args.thr,
            thr_after_acc=args.thrAfterOpEn,
            bins=args.p,
            less=args.less,
            chromosome_choice=args.chr,
            qid=args.q,
            ris2=args.rx,
            seq_file=args.os,
        )
    ...
    if on_predictions_file != "":
        ...
        try:
            min_p, min_na = add_update_on_targets(
                on_predictions_file,
                on_accessibility_file,
                pres,
                on_targets,
                min_p,
                min_na,
    ...
    t6 = time.time()
    sys.stderr.write(
        "#TIMETRACK: Save predictions and opening energies " + str(t6 - t5) + "\n"
    )
        ...
        if float(alpha) == 1.0:
            ...
            for gamma in args.gamma.split(";"):
                if (
                    ...
                    and (float(alpha), float(gamma)) not in ab_list
                ):
    ...
    if args.tp is not None:
        pres, on_targets = apply_tp_threshold(
            pres, on_targets, args.tp, min_p, min_na, args.thrAfterOpEn
        )

    t7 = time.time()
    sys.stderr.write(
        "#TIMETRACK: Percent threshold and alpha beta parsing " + str(t7 - t6) + "\n"
    )

    # Step 6.3: Get probabilities
    try:
        probs, probs_na, z_val, z_val_na, on_probs, on_probs_na = get_probabilities(
            pres, on_targets, min_p, min_na, ab_list, args.offPs
        )
    except:
        sys.stderr.write(
            "#ERROR: Cannot finish computing the partition function and probabilities\n"
        )
        sys.stderr.write("Unexpected error: " + str(sys.exc_info()) + "\n")
        pipeline_exit(failed=True)

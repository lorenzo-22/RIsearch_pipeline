...
class ProbabilityService:
    """Calculates off-target probabilities via Boltzmann partition functions."""
    ...
    def calculate_probabilities(
        ...
        """Calculate P(OT) for each candidate using a single partition function Z.

        Z = Σ(Expr_i · exp(-dG_total_i / RT)) + Expr_on · exp(-dG_total_on / RT)
        P(t) = W_t / Z
        """
        ...
        dG_expr = (pl.col("energy") + pl.col("opening_energy")) if "opening_energy" in df.columns else pl.col("energy")
        df = df.with_columns(dG_expr.alias("dG_total"))

        expr_val = pl.col("exp_value").fill_null(0.0) if "exp_value" in df.columns else pl.lit(1.0)
        ...
        df = df.with_columns(
            (pl.col("boltzmann_weight") / z_total if z_total > 0 else pl.lit(0.0)).alias("P_off_target")
        )
        ...
        if on_target_path and query_path:
            ...
            row_dict = {
                "chrom": "onTarget", "start": _start, "end": _end, "strand": _strand,
                "transcript_id": "onTarget", "gene_id": "onTarget", "energy": _hyb,
                "opening_energy": dG_open_on, "dG_total": dG_total_on,
                "exp_value": on_target_expression, "P_off_target": p_on_val,
            }
            ...
            for col in df.columns:
                ...
                elif df.schema[col] != ont_df.schema[col]:
                    ont_df = ont_df.with_columns(pl.col(col).cast(df.schema[col]))
    ...
    def calculate_probabilities_per_sirna(
        ...
        """Calculate P(OT) per siRNA using independent per-siRNA partition functions.

        Two on-target modes:
        - In-predictions: on-target rows already in df, tagged by gene/transcript match.
        - FASTA: on-target energy pre-computed separately; rows excluded from off-target Z.
        """
        if "energy" not in df.columns:
            logger.warning("No 'energy' column found. Cannot calculate probabilities.")
            return df, {}
        ...
        if in_predictions_mode:
            ...
            for sirna_id, target_id in local_map.items():
                ...
                tag_conditions.append((pl.col("sirna_id") == sirna_id) & id_match)
            ...
            logger.info(f"In-predictions on-target mode: tagged {n_on} rows as on-target")
        ...
        # Anchor clamping to the minimum energy from the raw RIsearch file (all genome
        # hits, including those that don't overlap any annotated transcript). Falls back
        # to the post-intersection minimum when raw_e_min is absent.
        e_min_col = "raw_e_min" if "raw_e_min" in df.columns else "energy"
        min_energies = df.group_by("sirna_id").agg(pl.col(e_min_col).min().alias("E_min"))
        ...
        if in_predictions_mode:
            ...
            for suffix, _, _ in calc_configs:
                ...
                for sirna_id, data in on_target_data_fasta.items():
                    ...
                    on_target_weight_metadata.setdefault(sirna_id, {})[f"W_on{suffix}"] = w_on
                    on_target_weight_metadata.setdefault(sirna_id, {})[f"W_on_noacc{suffix}"] = w_on_noacc
            ...
            logger.info(f"FASTA mode: added on-target weights for {len(on_target_data_fasta)} siRNAs")
    ...
    def format_legacy_summary(
        ...
        """Format partition-function metadata as a .summary file block."""
        ...
        for suffix, p1, p2 in suffixes:
            if suffix == "":
                preamble = "# For alpha=1.0 and gamma=1.0;"
            elif suffix.startswith(":theta="):
                preamble = f"# For theta={p1};"
            else:
                preamble = f"# For alpha={p1} and gamma={p2};"
    ...
    def calculate_legacy_format(
        ...
        """Calculate probabilities in legacy gw.results format."""
        if "energy" not in df.columns:
        ...
        for alpha, gamma in alpha_gamma_pairs:
            ...
            on_target_stats[(alpha, gamma)] = {
                "Pon": w_on / z_total if z_total > 0 else 0.0,
                "Pon_noacc": w_on_noacc / z_total_noacc if z_total_noacc > 0 else 0.0,
                "Poff": z_off / z_total if z_total > 0 else 0.0,
                "Poff_noacc": z_off_noacc / z_total_noacc if z_total_noacc > 0 else 0.0,
                "Z": w_on,
                "Z_noacc": w_on_noacc,
                "Zoff": z_off,
            ...
            df_agg = df_agg.with_columns([
                (pl.col("W_transcript") / z_total if z_total > 0 else pl.lit(0.0)).alias("P_transcript"),
                (pl.col("W_transcript_noacc") / z_total_noacc if z_total_noacc > 0 else pl.lit(0.0)).alias("P_transcript_noacc"),
            ])
        ...
        if verbose:
            ...
            for alpha, gamma in alpha_gamma_pairs:
                header_parts.extend([f"alpha:{alpha},gamma:{gamma}", f"alpha:{alpha},gamma:{gamma}(NoAcc)"])
    ...
    def _annotate_opening_energy(self, df: pl.DataFrame) -> pl.DataFrame:
        """Lookup opening energy for each prediction row.

        Path 1 (Parquet join): O(N log N). Path 2 (mmap profile): O(N) via vectorized gather.
        """
        if df.height == 0:
            return df
    ...
    def _calculate_on_target_components(
        ...
        """Return (dG_hyb, dG_open) for the on-target sequence."""
        if risearch_path:
        ...
        if accessibility_path and accessibility_path.exists() and t_end > 0:
            try:
                ...
                if "openen" in accessibility_path.name:
                    ...
                    profile = raw_data.reshape(-1, 30) if len(raw_data) % 30 == 0 else raw_data
                else:
                    profile = np.array([])
                ...
                if profile.size > 0:
                    if profile.ndim == 2:
                        col_idx = max(0, min(interaction_len, profile.shape[1]) - 1)
                        target_idx = (t_end - 1) if strand == "+" else start0
                        if 0 <= target_idx < profile.shape[0] and start0 >= 0 and t_end <= profile.shape[0]:
                            val = profile[target_idx, col_idx]
                            dG_open = int(round(float(val))) / 10.0 if profile.dtype == np.uint8 else int(round(float(val) * 10.0)) / 10.0
                        else:
                            dG_open = 10.0
                    else:
                        target_idx = (t_end - 1) if strand == "+" else start0
                        if 0 <= target_idx < profile.shape[0]:
                            ...
                            dG_open = float(val) / 10.0 if profile.dtype == np.uint8 else int(round(float(val) * 10.0)) / 10.0
                        else:
                            dG_open = 10.0

            except Exception as e:
                logger.error(f"Failed to calculate on-target accessibility from file: {e}")
                dG_open = 0.0

        elif t_end > 0:
            try:
                from RIsearch_pipeline.services.helpers import read_fasta
                sequence = next((seq for _, seq in read_fasta(on_target_path)), None)
    ...
    def _run_risearch_binary(self, query_path: Path, target_path: Path) -> tuple[float, int, int, str]:
